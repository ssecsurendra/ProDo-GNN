...
# Kernel to calculate cosine dissimilarity for each edge
dissimilarity_kernel = cp.RawKernel(r'''
extern "C" __global__
void calculate_cosine_dissimilarity(
    ...
    for (long e = edge_start + threadIdx.x; e < edge_end; e += blockDim.x) {
        ...
        float similarity = fmaxf(raw_cosine, 0.0f);
        // Calculate dissimilarity
        edge_weights[e] = 1.0f - similarity;
    }
}
''', 'calculate_cosine_dissimilarity')
...
if __name__ == "__main__":
    ...
    print(f"Calculating DISSIMILARITY-WEIGHTED Eigenvector Centrality for {args.dataset}.")
    ...
    print("Calculating edge weights using cosine dissimilarity...")
    ...
    dissimilarity_kernel((blocks_per_grid,), (threads_per_block,), 
                      (features_cp, row_ptr_cp, col_idx_cp, edge_weights_cp, feature_dim, num_nodes))
    ...
    # --- 4. Save results ---
    out_dir = "dissimilarity-weighted-centrality"
    os.makedirs(out_dir, exist_ok=True)
    filename_centrality = os.path.join(out_dir, f"{args.dataset}_dissimilarity_eigen-centrality.npy")
    filename_sorted_col_idx = os.path.join(out_dir, f"{args.dataset}_dissimilarity_eigen_sorted-col-index.npy")
    ...
    print(f"✅ Dissimilarity-weighted eigenvector centrality saved to {filename_centrality}")
