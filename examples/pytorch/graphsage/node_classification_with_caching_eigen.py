...
def train(args, device, g, dataset, model, num_classes, centrality_vals, feat_cache, node_map, cached_nodes_mask, label_cache):
    ...
    for epoch in range(args.epoch):
        ...
        for it, (input_nodes, output_nodes, blocks) in enumerate(
            ...
            current_input_nodes = blocks[0].srcdata[dgl.NID]
            current_output_nodes = blocks[-1].dstdata[dgl.NID]
            y = label_cache[current_output_nodes]
            if args.mode == 'mixed' and feat_cache is not None:
                is_cached = cached_nodes_mask[current_input_nodes]
                cached_part = current_input_nodes[is_cached]
                hit_ratio = hit_ratio + len(cached_part)/len(current_input_nodes)
                non_cached_part = current_input_nodes[~is_cached]
        ...
        epoch_line = "Epoch {:05d} | Loss {:.4f} | Accuracy {:.4f} | Time : {:.4f} | Loop_Time : {:.4f} | Model_Time : {:.4f} | x_y_time : {:.4f} | pred_time : {:.4f} | backward_time : {:.4f} | optim_time : {:.4f} | Hit_ratio : {:.4f} ".format(epoch, total_loss / (it + 1), acc.item(), execution_time, iteration_time1, model_time1, x_y_time, pred_time, backward_time, optim_time, hit_ratio/(it+1) )
...
if __name__ == "__main__":
    ...
    # row_ids = which node each edge belongs to
    ...
    # num_nodes_degree_greater_than_20 = len(out_degrees[out_degrees > 50])
    # percentage_degrre_greater_than_20 = (num_nodes_degree_greater_than_20/len(out_degrees))*100
    ...
    # print("Total number of nodes with out-degree greater than 20: ", num_nodes_degree_greater_than_20)
    # print("Percentage: ", percentage_degrre_greater_than_20)
    ...
    #
    # # sort training nodes by degree centrality (descending order)
    # sorted_idx = np.argsort(-train_degrees)  
    #
    # # how many to keep (top 70%)
    ...
    #
    # # replace in dataset
    # dataset.train_idx = top_train_idx
    # # Apply threshold
    # threshold = 0.0005
    # mask = degree_vals > threshold
    #
    # # Filter training nodes by degree centrality
    # train_idx = dataset.train_idx
    # print("Original training nodes:", len(train_idx))
    # filtered_train_idx = train_idx[mask[train_idx]]
    #
    # # Replace train_idx in dataset for training
    # dataset.train_idx = filtered_train_idx
    # print(f"Filtered training nodes: {len(filtered_train_idx)}") 
    ...
    if args.mode == 'mixed':
        ...
        if num_nodes_to_cache > 0:
            ...
            sorted_degrees, sorted_indices = torch.sort(centrality_vals, descending=True)
            ...
            feat_cache = g.ndata['feat'][high_degree_nodes.to('cpu')].to(device)
            print(f"Cached features for {num_nodes_to_cache} nodes on GPU based on degree.")
            label_cache = g.ndata['label'].to(device)

    
    epoch_lines = train(args, device, g, dataset, model, num_classes, centrality_vals, feat_cache, node_map, cached_nodes_mask, label_cache)
