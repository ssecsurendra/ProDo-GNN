...
def train(args, device, g, dataset, model, num_classes, centrality_vals):
    ...
    print("\n--- Timing Original Dominating Set Computation ---")
    ...
    print("\n--- Timing Optimized Dominating Set Computation ---")
    start_time = time.time()
    ...
    print(f"Size of optimized dominating set: {len(dominating_set)}")
    if optimized_time > 0:
    ...
    for epoch in range(args.epoch):
        ...
        epoch_line = (f"Epoch {epoch:05d} | Loss {total_loss / (it + 1):.4f} | "
                      f"Accuracy {acc.item():.4f} | Time {execution_time:.4f}")
        print(epoch_line)
...
if __name__ == "__main__":
    ...
    # For OGBN datasets, the graph is already directed and doesn't have masks
