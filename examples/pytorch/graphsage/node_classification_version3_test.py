...
def train(args, device, g, 
          # cluster_id,
          dataset, model, num_classes, degree_vals):
...
if __name__ == "__main__":
    ...
    edge_ids = torch.arange(num_edges, dtype=torch.int64, device=device)
    sorted_col_idx = sorted_col_idx.to(torch.int64)
    #g = dgl.from_csr(indptr, sorted_col_idx)
    g = dgl.graph(('csr', (indptr, sorted_col_idx, edge_ids)), num_nodes=len(indptr) - 1, device=device)
    g.ndata.update(G.ndata)  # copy node features
    g.edata.update(G.edata)  # same order, no remapping needed
    del G
